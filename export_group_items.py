...
def get_count_groups(auth):
    REQUEST_DATA = '{ "jsonrpc": "2.0", "auth": "' + auth + '", "id": 1, "method": "hostgroup.get","params": {"countOutput": "true"} }'
...
while n < int(count_groups):
    REQUEST_DATA = '{ "jsonrpc": "2.0", "auth": "' + zabauth + '", "id": "' + str(i) + '", "method": "hostgroup.get","params": {"monitored_hosts ": "1"} }'
...
j = 1
for group in groups:
    REQUEST_DATA = '{ "jsonrpc": "2.0", "auth": "' + zabauthnew + '", "id": "' + str(j) + '", "method": "hostgroup.create","params": {"name": "' + group + '"} }'
